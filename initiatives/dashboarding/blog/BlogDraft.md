# Stop Context Switching: Dashboards That Know Your Kubernetes Architecture
...
**The missing piece:** Whether you're running a separate dashboarding tool or doing without, there's a gap—custom dashboards that understand your architecture and integrate with your troubleshooting workflow.
...
**Today, we're releasing Dashboarding for SUSE Observability**—our most requested feature, built to close that gap.
...
Drop that query into a widget, add a `${namespace}` variable, and you've got a dashboard that works across every namespace—with each data point linking back to the actual pod.
